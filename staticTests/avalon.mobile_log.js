...
new function() {// jshint ignore:line
    ...
    function touchmove(event) {
        ...
        if (_isPointerType && !isPrimaryTouch(event)) return
        /*
            现象：1.如果 触发了touchmove && 不阻止默认行为 ，touchEnd 无法被触发；2.android浏览器 不阻止默认行为 时, touchmove 只会被触发一次【偶尔是两次】, ios可以连续触发,而且触发时机十分不准确;
            总结：在4.0.X的机器，如果touchMove触发了，就不会触发thoucEnd， 要在 touchmove加上 event.preventDefault(); 
